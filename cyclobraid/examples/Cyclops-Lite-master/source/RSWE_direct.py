...
class Solvers:
    ...
    @staticmethod
    def coarse_propagator_reuse(control, expInt, st, U_hat):
        """
        Implements the coarse propagator used in the APinT method. Can handle any implemented methods
        for the linear and nonlinear operator. Calls through to appropriate methods. Differs from the
        non_asymptotic version in its call to the average force computation.
        ...
        **Returns**
        ...
        while t < 0.9999999999999*control['final_time']:
            ...
            U_hat_new = strang_splitting_reuse(U_hat_old, dt, control, expInt, st, dissipative_exponential, compute_average_force)
            U_hat_new = expInt.call_(U_hat_new, dt)
...
def strang_splitting(U_hat, delta_t, control, expInt, st, linear, nonlinear):
    ...
    **Returns**
    ...
    if nonlinear is compute_average_force: 
        if control['iter'] not in control['UhatNL1']:
            control['UhatNL1'][control['iter']] = { }
        if control['iter'] not in control['UhatNL2']:
            control['UhatNL2'][control['iter']] = { }
        control['UhatNL1'][control['iter']][control['index']] = U_NL_hat1
        control['UhatNL2'][control['iter']][control['index']] = U_NL_hat2

    U_hat_new = linear(control, expInt, U_hat_new, 0.5*delta_t)
    return U_hat_new

def strang_splitting_reuse(U_hat, delta_t, control, expInt, st, linear, nonlinear):
    """
    Propagates a solution for arbitrary linear and nonlinear operators over a timestep
    delta_t by using Strang Splitting.
    ...
    **Returns**
    ...
    #U_NL_hat = nonlinear(U_hat_new, control, st, expInt)
    #U_NL_hat1 = -delta_t*U_NL_hat
    U_NL_hat1 = control['UhatNL1'][control['iter']-1][control['index']]

    #U_NL_hat = nonlinear(U_hat_new + 0.5*U_NL_hat1, control, st, expInt)
    #U_NL_hat2 = -delta_t*U_NL_hat
    U_NL_hat2 = control['UhatNL2'][control['iter']-1][control['index']]
