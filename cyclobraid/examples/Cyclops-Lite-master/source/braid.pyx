...
cdef extern from "braid_status.h":
    ...
    ##
    # Wrap Status Routines
    int braid_StatusGetCallingFunction (braid_StepStatus status, int *cfunction_ptr)
...
cdef extern from "_braid.h":
    int _braid_GetCFactor(braid_Core core, int level, int *cfactor_ptr)
